...
func main()  {
	...
	//数组的申明
	var courses1 [10] string
	var courses2 = [5]string{"django", "scrapy", "tornado"}
	courses3 := [5]string{"django", "scrapy", "tornado"}
	...
	e := [5]int{4:100} // 下标4设置为100，其他都用默认值
	fmt.Println(e)
	...
	//在go语言中，courseA和courseB都是数组，但是不是同一种类型。因为它们的长度不一样
	fmt.Printf("%T\n", courseA)
	fmt.Printf("%T\n", courseB)
	//如果courseA和courseB是一种类型的话 为什么前面要加一个数组， 长度不一样的数组类型是不一样
	//正是基于这些，在go语言中函数传递参数的时候，数组作为参数 实际调用的时候是值传递
	//printArray(courseA) // 传不过去，会失败
	printArray(courseB) // 不影响原来的数组的值，会拷贝一份
	fmt.Println(courseB)
}